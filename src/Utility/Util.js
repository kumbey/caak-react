...
export function generateFileUrl(file) {
    if (file)
        return "https://" + file.bucket + ".s3." + file.region + ".amazonaws.com/" +
                            file.level + "/" + file.id + "." + file.ext
    return null;
}

export function decodeURL(string){
    var id = CryptoJS.AES.decrypt(decodeURIComponent(string), Configure.urlEncodeKey).toString(CryptoJS.enc.Utf8);
    return id;
}

export function encodeURL(string){
    return encodeURIComponent(CryptoJS.AES.encrypt(string, Configure.urlEncodeKey));
}
...
export function generateTimeAgo(date) {
    ...
    const diff = today.diff(postdate, ["years", "months", "days", "hours", "minutes", "seconds", "milliseconds"]);
