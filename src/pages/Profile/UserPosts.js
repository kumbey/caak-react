...
export default function UserPosts({ userId, type, card }) {
    ...
    const subscrib = () => {
        subscriptions.onPostByUserPosts = API.graphql({
            query: onPostByUser,
            variables: {
                user_id: userId,
                status: type,
    ...
    useEffect(() => {
        if (subscriptionPosts) {
            if (
                !userPosts.find((item) => item?.id === subscriptionPosts?.id)
            ) {
            if (subscriptionPosts.status === type)
                setUserPosts((prev) => [subscriptionPosts, ...prev]);
            } else {
